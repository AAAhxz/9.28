...
<script type="text/javascript">
			...
			<script type="text/javascript">
	//查询栏不清空设置Js
	window.onload = function chaxun(){
	var campus=${campus};
	var username = ${username};
	var department= ${department};
	var position = ${position};
	alert(campus);
	$("#cha_campus").val(campus);
...
<body>
	<div id="wrapper">
		...
		<div id="page-wrapper">
							...
							<tbody>
								...
								<c:forEach items="${data}" var="row">
									<tr>
										...
										<td>${row.telnumber}</td>
										<td>
											<a href="#" class="btn btn-primary btn-xs"data-toggle="modal" data-target="#yibanEditDialog"
											onclick="edityiban(${row.id})">修改</a> 
											<a class="btn btn-danger btn-xs" onclick="deleteyiban(${row.id})">删除</a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
			...
			<!-- 创建成员模态框 -->
			<div class="modal fade" id="newYibanDialog" tabindex="-1"
